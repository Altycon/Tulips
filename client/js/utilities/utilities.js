...
function setUserLanguage(DEVICE_LANGUAGE){
    if(DEVICE_LANGUAGE){
        return DEVICE_LANGUAGE.split('-')[0] || 'en';
    }else{
        return false;
    }
}
...
// windows
function openWindow(window_element){
    lockBody();
    window_element.classList.add('open');
    return window_element;
}
function closeWindow(window_element){
...
function generatePassKey(){
   return random(100000, 999999, true)
}
